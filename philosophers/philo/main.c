...
/*   Updated: 2023/08/09 17:48:13 by atalaver         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char **argv)
{
	...
	if (ft_load_datos(&datos, argc, argv))
		return (ft_free(philos, &datos), 1);
	datos.forks = ft_init_mutex(datos);
	if (!datos.forks)
		return (ft_free(philos, &datos), printf("Error::Create mutex\n"), 1);
	philos = ft_create_philos(&datos);
	if (!philos)
		return (ft_free(philos, &datos), printf("Error::Malloc philos\n"), 1);
